...
new Vue({
    ...
    created() {
        this.initPeople(this.peopleUrl);
    },
    methods: {
        initPeople: function(url){
            // this.swData.results.forEach(element => {
            //     this.people.push(element);
            // });
            // this.pageNums = this.getPageNums(this.swData.count);
            ...
            $.get(url, function(data, status){
                ...
                if(status === "success"){
                    ...
                    data.results.forEach(element => {
                        self.people.push(element);
                    });
                    if(self.pageNums.length === 0){
                        self.pageNums = self.getPageNums(self.data.count);
                    }
                }
            });
        },
        getPageNums: function(count){
            var nums = (count / 10) + 1;
            var pages = [];
        ...
        changePage: function(e){
            console.log(e);
            this.currPage = e;
            this.initPeople(peopleUrl + "?page=" + e.toString());
        }
    }
});
